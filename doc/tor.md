...
It is possible to run SSS as a Tor hidden service, and connect to such services.
...
Run SSS behind a Tor proxy
----------------------------------

The first step is running SSS behind a Tor proxy. This will already make all
...
```
./sssd -onion=127.0.0.1:9050 -onlynet=tor -listen=0 -addnode=dnetzj6l4cvo2fxy.onion:989
```

In a typical situation, this suffices to run behind a Tor proxy:
```
./sssd -proxy=127.0.0.1:9050
```

Run a SSS hidden server
-------------------------------
...
your sssd's P2P listen port (51472 by default).
```
-externalip=X   You can tell sss about its publicly reachable address using
                this option, and this can be a .onion address. Given the above
                configuration, you can find your onion address in
                /var/lib/tor/sss-service/hostname. Onion addresses are given
...
```
./sssd -proxy=127.0.0.1:9050 -externalip=dnetzj6l4cvo2fxy.onion:989 -listen
```
...
```
./sssd -onion=127.0.0.1:9050 -externalip=dnetzj6l4cvo2fxy.onion:989 -discover
```

List of known SSS Tor relays
------------------------------------
```
